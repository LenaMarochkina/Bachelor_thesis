...
best_index_DT <- which.max(grid_results$AUC)
...
cat(sprintf("Best AUC: %.4f\n\n", grid_results$AUC[best_index_DT]))

# Predict with best model
best_probs_DT <- predict(best_model_DT, validation_data, type = "prob")[, 2]
...
axis(side = 1, las = 2, labels = names(Labels),
     at = 1:length(Labels), cex.axis = 0.5)
...
```
Boruta performed 42 iterations in 2 hours.
 95 attributes confirmed important: ABP_DIA, ABP_MEAN,
ABP_SYS, AGE_AT_ADMISSION, ARTERIAL_LINE and 90 more;
 2 attributes confirmed unimportant: REQUEST_CDIFF,
REQUEST_VRE;
 1 tentative attributes left: ICD9_4240;
...
# ------------------------------------------------------------------------
# SHAP-BASED FEATURE SELECTION AFTER UNDERSAMPLING
# ------------------------------------------------------------------------
...
shap_df <- data.frame(Feature = names(shap_importance), Importance = shap_importance)
shap_df <- shap_df[order(-shap_df$Importance), ]
shap_df$ExplainedVariance <- shap_df$Importance / sum(shap_df$Importance)
shap_df$CumulativeVariance <- cumsum(shap_df$ExplainedVariance)

# Select top features explaining 80% of SHAP importance
top_features <- shap_df$Feature[shap_df$CumulativeVariance <= 0.9]

# DEBUG: Check features retained
cat("Number of SHAP-selected features:", length(top_features), "\n")
print(head(top_features))

# Subset training and validation data to SHAP-selected features
train_selected_matrix <- as.matrix(train_matrix[, top_features])
validation_selected_matrix <- as.matrix(validation_matrix[, top_features])

train_xgb_shap <- xgb.DMatrix(data = train_selected_matrix,
                              label = as.numeric(train_data_balanced$SURVIVAL_FLAG) - 1)
val_xgb_shap <- xgb.DMatrix(data = validation_selected_matrix,
                            label = as.numeric(validation_data_xgb$SURVIVAL_FLAG) - 1)

# ------------------------------------------------------------------------
# GRID SEARCH ON SHAP-SELECTED FEATURES (AUC-based Selection)
# ------------------------------------------------------------------------

# Define a small but meaningful grid
param_grid_shap <- expand.grid(
  max_depth = c(8),
  eta = c(0.1),
  gamma = c(3),
  subsample = c(0.7),
...
best_auc_params <- NULL
best_predictions_shap <- NULL

cat("\n🔍 Grid search with SHAP-selected features...\n")

for (i in 1:nrow(param_grid_shap)) {
  params <- param_grid_shap[i, ]
  
  xgb_params_shap <- list(
    objective = "binary:logistic",
    eval_metric = "auc",
    max_depth = params$max_depth,
    eta = params$eta,
  ...
  set.seed(123)
  model_shap <- xgb.train(
    params = xgb_params_shap,
    data = train_xgb_shap,
    nrounds = 100,
  ...
  pred <- predict(model_shap, val_xgb_shap)
  auc_val <- auc(roc(validation_data_xgb$SURVIVAL_FLAG, pred))
  
  cat(sprintf("Config %d | AUC: %.4f | Params: %s\n", i, auc_val, paste(params, collapse = ", ")))
  
  if (auc_val > best_auc) {
    best_auc <- auc_val
    best_model_shap <- model_shap
    best_auc_params <- params
    best_predictions_shap <- pred
  }
}

cat("\n✅ Best AUC configuration (SHAP features):\n")
print(best_auc_params)
cat("Best AUC:", round(best_auc, 4), "\n")
# ------------------------------------------------------------------------
# EVALUATE BEST AUC MODEL ON SHAP-SELECTED FEATURES
# ------------------------------------------------------------------------

val_class_shap <- ifelse(best_predictions_shap > 0.5, 1, 0)
val_class_shap <- factor(val_class_shap, levels = levels(validation_data_xgb$SURVIVAL_FLAG))

cat("\n📊 Best SHAP-Selected Model Performance:\n")
print_metrics(best_model_shap, validation_data_xgb, best_predictions_shap, val_class_shap)

# Save model json
xgb.save(best_model_shap, "final_xgb_model_shap_90.json")
...
Explained variance 0.8 (33 features)
Best metrics max depth 8 eta 0.05 gamma 0 subsample 1 colsample 0.7
threshold 0.5
Accuracy: 0.8911115 
Sensitivity (Recall): 0.9281191 
Specificity: 0.6006975 
Precision: 0.9480254 
F1-score: 0.9379667 
AUC: 0.8936519 

Explained variance 0.9 (features)
Best metrics max depth 8 eta 0.1 gamma 3 subsample 0.7 colsample 0.7
threshold 0.5
Accuracy: 0.8884509 
Sensitivity (Recall): 0.9211199 
Specificity: 0.6320837 
Precision: 0.9515666 
F1-score: 0.9360957 
AUC: 0.8979927 
...
Boruta performed 39 iterations in 1.15669 hours.
 65 attributes confirmed important: AGE_AT_ADMISSION,
BG_LAST_ALVEOLAR_ARTERIAL_GRADIENT, BG_LAST_CALCULATED_BICARBONATE_WHOLE_BLOOD,
BG_LAST_CALCULATED_TOTAL_CO2, BG_LAST_CARBOXYHEMOGLOBIN and 60 more;
 8 attributes confirmed unimportant: CH_LAST_CRP, GENDER_F, GENDER_M,
H_LAST_SEDIMENTATION_RATE, ICD9_41071 and 3 more;
 1 tentative attributes left: ICD9_4241;

Elbow point detected at feature number 18

 18 features
Best params max depth 10 eta 0.05 gamma 0 subsample 0.7 colsample 0.8
threshold 0.3
Accuracy: 0.8876626 
Sensitivity (Recall): 0.9362293 
Specificity: 0.5065388 
Precision: 0.9370622 
F1-score: 0.9366455 
AUC: 0.8696694 
