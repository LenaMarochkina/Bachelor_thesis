...
prescriptions_survival_numbers <- prescriptions_survival_numbers %>%
  mutate(
    SURVIVAL_FLAG = as.factor(SURVIVAL_FLAG)
  )

isolations_survival_outcome <- isolations_survival_outcome %>%
  mutate(
    SURVIVAL_FLAG = as.factor(SURVIVAL_FLAG)
  )

procedures_with_duration_survival_outcome <- procedures_with_duration_survival_outcome %>%
  mutate(
    SURVIVAL_FLAG = as.factor(SURVIVAL_FLAG)
  )

procedures_binary_survival_outcome_flags <- procedures_binary_survival_outcome_flags %>%
  mutate(
    SURVIVAL_FLAG = as.factor(SURVIVAL_FLAG)
  )
...
survival_length <- survival_length %>%
  # add ATC_ before column names
  left_join(prescriptions_transformed_survival, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG")) %>%
  # All NA's change to 0 in columns from prescriptions_transformed_death_outcome
  mutate(across(starts_with("ATC"), ~ as.numeric(as.character(.)))) %>%  # Convert factors to numeric
  mutate(across(starts_with("ATC"), ~ replace_na(., 0))) %>%  # Replace NA with 0
  mutate(across(starts_with("ATC"), ~ as.factor(.)))
...
survival_length <- survival_length %>%
  left_join(labs_blood_gas_survival_last, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG"))
...
survival_length <- survival_length %>%
  left_join(hematology_survival_last, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG"))
...
survival_length <- survival_length %>%
  left_join(chemistry_survival_last, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG")) %>%
  # All NA's change to 0 in columns from prescriptions_transformed_death_outcome
  mutate(across(starts_with("LAST"), ~ as.numeric(as.character(.)))) %>%  # Convert factors to numeric
  mutate(across(starts_with("LAST"), ~ replace_na(., 0))) %>%  # Replace NA with 0
  mutate(across(starts_with("LAST"), ~ as.factor(.)))
...
survival_length <- survival_length %>%
  left_join(procedures_with_duration_survival_long, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG"))
