...
print(intersection_gas_hematology)
print(intersection_gas_chemistry)
print(intersection_hematology_chemistry)
...
Perform data transformation (Blood Gas).
...
labs_blood_gas_combined <- labs_blood_gas %>%
  ...
  select(-CHARTTIME, -INTUBATED, -`SPECIMEN TYPE`, -HEMOGLOBIN, -GLUCOSE) %>%
  distinct()
...
labs_blood_gas_combined <- labs_blood_gas_combined %>%
  rename_with(~ gsub(" ", "_", .)) %>%
  rename_with(~gsub(",_", "_", .))

```

```{r Descriptive statistics for procedures for categorical vars (Laboratory Data (Blood gas)}

# Summarize the statistics for each numeric variable
labs_blood_gas_combined %>%
  select(-VISIT_NUM) %>%
  select(where(is.numeric)) %>% 
  summarise(across(everything(), statistics)) %>%
  pivot_longer(cols = everything(), names_sep = "__", names_to = c("Variable", "Stat")) %>%
...
numeric_columns <- labs_blood_gas_combined %>%
  select(-VISIT_NUM) %>%
  select(where(is.numeric)) %>%
  names()
...
outlier_summary <- lapply(numeric_columns, function(col) {
  # Find outliers
  outlier_data <- find_outliers(labs_blood_gas_combined, col)
  
  # Summary stats
  total_outliers <- nrow(outlier_data)
  total_values <- nrow(labs_blood_gas_combined)
...
```
All outliers levels are less than 5 % (except pO2).
...
Perform data transformation (Hematology).

``` {r Hematology Data transformation}

unique_labels_list <- hematology %>%
  group_by(LABEL) %>%
  summarise(Count = sum(!is.na(VALUENUM)), .groups = "drop") %>%
  filter(Count > 1000) %>%
  arrange(desc(Count)) %>%
...
# add HEMOGLOBIN and GLUCOSE label rows from labs_blood_gas and D-DIMER from chemistry
# Filter for HEMOGLOBIN and GLUCOSE from labs_blood_gas
hemoglobin_glucose_data <- labs_blood_gas %>%
  filter(LABEL %in% c("HEMOGLOBIN", "GLUCOSE"))

# Filter for D-DIMER from chemistry
d_dimer_data <- chemistry %>%
  filter(LABEL == "D-DIMER")

# Combine the filtered data with the hematology dataset
hematology <- hematology %>%
  bind_rows(hemoglobin_glucose_data, d_dimer_data) %>%
  distinct()
...
hematology_combined <- hematology %>%
  group_by(SUBJECT_ID_COMPOSE, VISIT_NUM) %>%
  summarise(across(
    everything(), 
    ~ if (all(is.na(.))) NA else na.omit(.)[1], # Keep the first non-NA value for each column
...
# Rename columns to replace spaces and commas
hematology_combined <- hematology_combined %>%
  rename_with(~ gsub(" ", "_", .)) %>%
  rename_with(~gsub(",_", "_", .)) %>%
  rename(
    INR_PT = `INR(PT)`, 
...
```{r Descriptive statistics for procedures for categorical vars (Laboratory Data (Blood as)}
...
# Get numeric column names
numeric_columns <- hematology_combined %>%
  select(-VISIT_NUM) %>%
  select(where(is.numeric)) %>%
  names()
...
outlier_summary <- lapply(numeric_columns, function(col) {
  # Find outliers
  outlier_data <- find_outliers(hematology_combined, col)
  
  # Summary stats
  total_outliers <- nrow(outlier_data)
  total_values <- nrow(hematology_combined)
  percentage_outliers <- (total_outliers / total_values) * 100
...
All outliers levels are less than 5 %.

### 7.2.4. Descriptive statistics (Laboratory Data (Chemistry))

Perform data transformation (Chemistry).

``` {r Chemistry Data transformation}
...
```{r Descriptive statistics for numerical vars (Laboratory Data (Chemistry)}
...
# Get numeric column names
numeric_columns <- chemistry_combined %>%
  select(-VISIT_NUM) %>%
  select(where(is.numeric)) %>%
  names()
...
outlier_summary <- lapply(numeric_columns, function(col) {
  # Find outliers
  outlier_data <- find_outliers(chemistry_combined, col)
  
  # Summary stats
  total_outliers <- nrow(outlier_data)
  total_values <- nrow(chemistry_combined)
  percentage_outliers <- (total_outliers / total_values) * 100
...
Outliers are within normal range.
...
```{r Death Status by Laboratory Data (Hematology)}
hematology_death_outcome <- hematology_combined %>%
  left_join(Death_status_outcome, by = "SUBJECT_ID_COMPOSE") %>%
  select(-SUBJECT_ID) %>%
  filter(!is.na(SURVIVAL_FLAG))
...
# Visualise how differ all numeric vars according to death status
# Filter numeric variables with sufficient variance
numeric_vars <- hematology_death_outcome %>%
  select(-SUBJECT_ID_COMPOSE, -SURVIVAL_FLAG, -VISIT_NUM) %>%
  select(where(is.numeric)) %>%
  summarise(across(everything(), ~ var(., na.rm = TRUE))) %>%
  pivot_longer(everything(), names_to = "Variable", values_to = "Variance") %>%
...
# Filter data for plotting
filtered_data <- hematology_death_outcome %>%
  select(SUBJECT_ID_COMPOSE, VISIT_NUM, SURVIVAL_FLAG, all_of(numeric_vars)) %>%
  group_by(SURVIVAL_FLAG, VISIT_NUM) %>%
  summarise(across(everything(), ~ mean(., na.rm = TRUE), .names = "Mean_{.col}"), .groups = "drop")
...
for (variable in unique(melted_data$Variable)) {
  plot <- ggplot(melted_data %>% filter(Variable == variable), aes(x = VISIT_NUM, y = Value, color = factor(SURVIVAL_FLAG))) +
    geom_line(size = 1) +
    geom_point(size = 2, alpha = 0.7) +
    scale_color_manual(values = c("0" = color_palette[5], "1" = color_palette[20])) +
    labs(
      title = paste("Mean of", variable, "by Visit Number and Survival Flag"),
      x = "Visit Number",
      y = "Mean Value",
...
```{r Death Status by Laboratory Data (Chemistry)}
chemistry_death_outcome <- chemistry_combined %>%
  left_join(Death_status_outcome, by = "SUBJECT_ID_COMPOSE") %>%
  select(-SUBJECT_ID) %>%
  filter(!is.na(SURVIVAL_FLAG))
...
# Visualise how differ all numeric vars according to death status
# Filter numeric variables with sufficient variance
numeric_vars <- chemistry_death_outcome %>%
  select(-SUBJECT_ID_COMPOSE, -SURVIVAL_FLAG, -VISIT_NUM) %>%
  select(where(is.numeric)) %>%
  summarise(across(everything(), ~ var(., na.rm = TRUE))) %>%
  pivot_longer(everything(), names_to = "Variable", values_to = "Variance") %>%
...
# Filter data for plotting
filtered_data <- chemistry_death_outcome %>%
  select(SUBJECT_ID_COMPOSE, VISIT_NUM, SURVIVAL_FLAG, all_of(numeric_vars)) %>%
  group_by(SURVIVAL_FLAG, VISIT_NUM) %>%
  summarise(across(everything(), ~ mean(., na.rm = TRUE), .names = "Mean_{.col}"), .groups = "drop")
...
for (variable in unique(melted_data$Variable)) {
  plot <- ggplot(melted_data %>% filter(Variable == variable), aes(x = VISIT_NUM, y = Value, color = factor(SURVIVAL_FLAG))) +
    geom_line(size = 1) +
    geom_point(size = 2, alpha = 0.7) +
    scale_color_manual(values = c("0" = color_palette[19], "1" = color_palette[21])) +
    labs(
      title = paste("Mean of", variable, "by Visit Number and Survival Flag"),
      x = "Visit Number",
      y = "Mean Value",
...
labs_blood_gas_survival <- labs_blood_gas_combined %>%
...
blood_test_params <- setdiff(names(labs_blood_gas_survival), c("SURVIVAL", "SUBJECT_ID_COMPOSE", "VENTILATOR", "VISIT_NUM"))

for (param in blood_test_params) {
  ggplot(labs_blood_gas_survival, aes(x = SURVIVAL, y = .data[[param]])) +
    stat_bin2d(bins = 50) +
...
```{r Length of Survival by Laboratory Data (Hematology)}
hematology_survival <- hematology_combined %>%
  left_join(Survival_outcome, by = "SUBJECT_ID_COMPOSE") %>%
  select(-SUBJECT_ID) %>%
  filter(!is.na(SURVIVAL)) 
...
blood_test_params <- setdiff(names(hematology_survival), c("SURVIVAL", "SURVIVAL_FLAG", "SUBJECT_ID_COMPOSE", "VISIT_NUM"))

for (param in blood_test_params) {
  ggplot(hematology_survival, aes(x = SURVIVAL, y = .data[[param]])) +
    stat_bin2d(bins = 50) +
    scale_fill_gradient(low = "blue", high = "red") +
    labs(
      title = paste("Density Heatmap of", param, "vs Survival Length"),
...
```{r Length of Survival by Laboratory Data (Chemistry)}
chemistry_survival <- chemistry_combined %>%
  left_join(Survival_outcome, by = "SUBJECT_ID_COMPOSE") %>%
  select(-SUBJECT_ID) %>%
  filter(!is.na(SURVIVAL)) 

# Visualise each blood test parameter according to Survival Length
blood_test_params <- setdiff(names(chemistry_survival), c("SURVIVAL", "SURVIVAL_FLAG", "SUBJECT_ID_COMPOSE", "VISIT_NUM"))

for (param in blood_test_params) {
  ggplot(chemistry_survival, aes(x = SURVIVAL, y = .data[[param]])) +
    stat_bin2d(bins = 50) +
    scale_fill_gradient(low = "blue", high = "red") +
    labs(
      title = paste("Density Heatmap of", param, "vs Survival Length"),
...
```{r Length of ICU stay by Laboratory Data (Blood Gas)}
labs_blood_gas_LOS <- labs_blood_gas_combined %>%
  left_join(LOS_outcome, by = "SUBJECT_ID_COMPOSE") %>%
  select(-SUBJECT_ID)

# Visualise each blood test parameter according to ICU stay

blood_test_params <- setdiff(names(labs_blood_gas_LOS), c("LOS", "SUBJECT_ID_COMPOSE", "VENTILATOR", "VISIT_NUM", "ICUSTAY_ID"))

for (param in blood_test_params) {
  p <- ggplot(labs_blood_gas_LOS, aes(x = LOS, y = .data[[param]])) +
    stat_bin2d(bins = 50) +
    scale_fill_gradient(low = "blue", high = "red") +
    labs(
...
```{r Length of ICU stay by Laboratory Data (Hematology)}
hematology_LOS <- hematology_combined %>%
  left_join(LOS_outcome, by = "SUBJECT_ID_COMPOSE") %>%
  select(-SUBJECT_ID)

# Visualise each blood test parameter according to ICU stay

blood_test_params <- setdiff(names(hematology_LOS), c("LOS", "SUBJECT_ID_COMPOSE", "VISIT_NUM", "ICUSTAY_ID"))

for (param in blood_test_params) {
  p <- ggplot(hematology_LOS, aes(x = LOS, y = .data[[param]])) +
    stat_bin2d(bins = 50) +
    scale_fill_gradient(low = "blue", high = "red") +
    labs(
      title = paste("Density Heatmap of", param, "vs ICU stay length"),
...
```{r Length of ICU stay by Laboratory Data (Chemistry)}
chemistry_LOS <- chemistry_combined %>%
  left_join(LOS_outcome, by = "SUBJECT_ID_COMPOSE") %>%
  select(-SUBJECT_ID)

# Visualise each blood test parameter according to ICU stay

blood_test_params <- setdiff(names(chemistry_LOS), c("LOS", "SUBJECT_ID_COMPOSE", "VISIT_NUM", "ICUSTAY_ID"))

for (param in blood_test_params) {
  p <- ggplot(chemistry_LOS, aes(x = LOS, y = .data[[param]])) +
    stat_bin2d(bins = 50) +
    scale_fill_gradient(low = "blue", high = "red") +
    labs(
      title = paste("Density Heatmap of", param, "vs ICU stay length"),
...
```{r Death Status by Laboratory Data (Blood) Blood Gas correlation matrix}
...
correlation_data <- labs_blood_gas_death_outcome %>%
  select(where(is.numeric)) %>%
  select(-SURVIVAL_FLAG, -VISIT_NUM)
...
- pO2 and ALVEOLAR-ARTERIAL GRADIENT calculated correlation is equal to -0.92
- pO2 and Required O2 calculated correlation is equal to -0.92
- Calculated bicarbonate and calculated total CO2 calculated correlation is equal to 0.97
- Base excess and calculated total CO2 calculated correlationis equal to 0.92
- Glucose will be analysed as chemistry data

 Multicollinearity can lead to unstable or unreliable coefficient estimates.
 
Predictors to remove: Hematocrit, ALVEOLAR-ARTERIAL GRADIENT, Required O2, calculated total CO2.

```{r Death Status by Laboratory Data (Blood) Blood Gas linear regression}
# Define predictors to evaluate
predictors <- c("BASE_EXCESS", "FREE_CALCIUM", "LACTATE", "PCO2", "PH", "PO2", "POTASSIUM_WHOLE_BLOOD", "SODIUM_WHOLE_BLOOD", "CHLORIDE_WHOLE_BLOOD", "PEEP", "TEMPERATURE", "OXYGEN", "OXYGEN_SATURATION", "TIDAL_VOLUME", "VENTILATION_RATE", "CALCULATED_BICARBONATE_WHOLE_BLOOD", "O2_FLOW", "METHEMOGLOBIN", "CARBOXYHEMOGLOBIN")
...
# Define predictors to evaluate
predictors <- c("HEMOGLOBIN", "BASE_EXCESS", "FREE_CALCIUM", "LACTATE", "PCO2", "PH", "PO2", 
                "POTASSIUM_WHOLE_BLOOD", "SODIUM_WHOLE_BLOOD", "CHLORIDE_WHOLE_BLOOD", "GLUCOSE", 
                "PEEP", "TEMPERATURE", "OXYGEN", "OXYGEN_SATURATION", "TIDAL_VOLUME", 
                "VENTILATION_RATE", "CALCULATED_BICARBONATE_WHOLE_BLOOD", "O2_FLOW", 
                "METHEMOGLOBIN", "CARBOXYHEMOGLOBIN")

# Initialize an empty list to store results
result <- list()

labs_blood_gas_death_outcome <- labs_blood_gas_death_outcome %>%
  mutate(
    SURVIVAL_FLAG = as.factor(SURVIVAL_FLAG)
  )
...
for (predictor in predictors) {
  ...
  test_result <- perform_statistical_tests(labs_blood_gas_death_outcome, "SURVIVAL_FLAG", predictor, predictor)
...
rm(final_result, predictors, labs_blood_gas_death_outcome, test_result, auc_value, param, predictor)
```

#### 7.4.1.2. Hematology

```{r Death Status by Laboratory Data (Blood) Hematology correlation matrix}

# Assuming `labs_blood_gas_death_outcome` contains numeric data and non-numeric columns to be excluded
correlation_data <- hematology_death_outcome %>%
  select(where(is.numeric)) %>%
  select(-SURVIVAL_FLAG, -VISIT_NUM)
...
Predictors like contain nearly identical information:
- Hemoglobin, Hematocrit and RED_BLOOD_CELLS, calculated correlation is equal to 0.92
- INR(PT) and PT calculated correlation is equal to -0.92
- Lymphocytes and neutrophils calculated correlation is equal to -0.92

 Multicollinearity can lead to unstable or unreliable coefficient estimates.
 
Predictors to remove: Hematocrit, RED_BLOOD_CELLS, PT.

```{r Death Status by Laboratory Data (Blood) Hematology linear regression}
# Define predictors to evaluate
predictors <- c("HEMOGLOBIN", "INR_PT", "LYMPHOCYTES", "MCH", "MCHC", "MCV", "MONOCYTES", "NEUTROPHILS", "EOSINOPHILS", "PLATELET_COUNT", "PTT", "RDW", "WHITE_BLOOD_CELLS", "FIBRINOGEN_FUNCTIONAL", "BASOPHILS", "SEDIMENTATION_RATE", "RETICULOCYTE_COUNT_AUTOMATED", "D_DIMER")
...
for (predictor in predictors) {
  roc_obj <- roc(
    hematology_death_outcome$SURVIVAL_FLAG,
    hematology_death_outcome[[predictor]],
    plot = FALSE
  )
  roc_results[[predictor]] <- roc_obj
}
...
LYMPHOCYTES: AUC = 0.64
NEUTROPHILS: AUC = 0.604
PLATELET_COUNT: AUC = 0.611
RDW: AUC = 0.635

```{r Death Status by Laboratory Data (Blood) Hematology stat test}
# Define predictors to evaluate
predictors <- c("HEMOGLOBIN", "INR_PT", "LYMPHOCYTES", "MCH", "MCHC", "MCV", "MONOCYTES", "NEUTROPHILS", "EOSINOPHILS", "PLATELET_COUNT", "PTT", "RDW", "WHITE_BLOOD_CELLS", "FIBRINOGEN_FUNCTIONAL", "BASOPHILS", "SEDIMENTATION_RATE", "RETICULOCYTE_COUNT_AUTOMATED", "D_DIMER")
...
hematology_death_outcome <- hematology_death_outcome %>%
  mutate(
    SURVIVAL_FLAG = as.factor(SURVIVAL_FLAG)
  )
...
for (predictor in predictors) {
  # Perform the statistical test
  test_result <- perform_statistical_tests(hematology_death_outcome, "SURVIVAL_FLAG", predictor, predictor)
...
rm(final_result, predictors, hematology_death_outcome, test_result, auc_value, param, predictor)
```

#### 7.4.1.3. Blood Chemistry

```{r Death Status by Laboratory Data (Blood) Chemistry correlation matrix}

# Assuming `labs_blood_gas_death_outcome` contains numeric data and non-numeric columns to be excluded
correlation_data <- chemistry_death_outcome %>%
  select(where(is.numeric)) %>%
  select(-SURVIVAL_FLAG, -VISIT_NUM)
...
```{r Death Status by Laboratory Data (Blood) Chemistry linear regression}
# Define predictors to evaluate
...
for (predictor in predictors) {
  roc_obj <- roc(
    chemistry_death_outcome$SURVIVAL_FLAG,
    chemistry_death_outcome[[predictor]],
    plot = FALSE
  )
  roc_results[[predictor]] <- roc_obj
}
...
In the provided table, the following predictors should be included in future modeling:
CREATININE: AUC = 0.603
UREA_NITROGEN: AUC = 0.675
BICARBONATE: AUC = 0.625
ANION_GAP: AUC = 0.608
BILIRUBIN_TOTAL: AUC = 0.637
AST: AUC = 0.602
CRP: AUC = 0.618
LD: AUC = 0.629

```{r Death Status by Laboratory Data (Blood) Chemistry stat test}
# Define predictors to evaluate
predictors <- c("POTASSIUM", 
                      "SODIUM", 
                      "CREATININE", 
...
chemistry_death_outcome <- chemistry_death_outcome %>%
  mutate(
    SURVIVAL_FLAG = as.factor(SURVIVAL_FLAG)
  )
...
for (predictor in predictors) {
  # Perform the statistical test
  test_result <- perform_statistical_tests(chemistry_death_outcome, "SURVIVAL_FLAG", predictor, predictor)
...
```{r Length of Survival  by Laboratory Data (Blood) Blood Gas correlation matrix}
# Assuming `labs_blood_gas_survival` contains numeric data and non-numeric columns to be excluded
correlation_data <- labs_blood_gas_survival %>%
  select(where(is.numeric)) %>%
  select(-SURVIVAL_FLAG, -VISIT_NUM)
...
```{r Length of Survival  by Laboratory Data (Blood) Blood Gas HR }

# Define predictors to evaluate
predictors <- c("HEMOGLOBIN", "BASE_EXCESS", "FREE_CALCIUM", "LACTATE", "PCO2", "PH", "PO2", "POTASSIUM_WHOLE_BLOOD", "SODIUM_WHOLE_BLOOD", "CHLORIDE_WHOLE_BLOOD", "GLUCOSE", "PEEP", "TEMPERATURE", "OXYGEN", "OXYGEN_SATURATION", "TIDAL_VOLUME", "VENTILATION_RATE", "CALCULATED_BICARBONATE_WHOLE_BLOOD", "O2_FLOW", "METHEMOGLOBIN", "CARBOXYHEMOGLOBIN")
...
for (predictor in predictors) {
  ...
  hr_result <- HR_assessment("SURVIVAL", "SURVIVAL_FLAG", predictor, labs_blood_gas_survival)
...
rm(predictors, hr_results, hr_table, labs_blood_gas_survival, hr_result)
```

#### 7.4.2.2. Hematology

```{r Length of Survival  by Laboratory Data (Blood) Hematology correlation matrix}
# Assuming `labs_blood_gas_survival` contains numeric data and non-numeric columns to be excluded
correlation_data <- hematology_survival %>%
  select(where(is.numeric)) %>%
  select(-SURVIVAL_FLAG, -VISIT_NUM)
...
```{r Length of Survival  by Laboratory Data (Blood) Hematology HR }
...
for (predictor in predictors) {
  # Perform Hazard Ratio (HR) assessment for the current predictor
  hr_result <- HR_assessment("SURVIVAL", "SURVIVAL_FLAG", predictor, hematology_survival)
...
```{r Length of Survival  by Laboratory Data (Blood) Chemistry correlation matrix}
# Assuming `labs_blood_gas_survival` contains numeric data and non-numeric columns to be excluded
correlation_data <- chemistry_survival %>%
  select(where(is.numeric)) %>%
  select(-SURVIVAL_FLAG, -VISIT_NUM)
...
```{r Length of Survival  by Laboratory Data (Blood) Chemistry HR }
...
for (predictor in predictors) {
  # Perform Hazard Ratio (HR) assessment for the current predictor
  hr_result <- HR_assessment("SURVIVAL", "SURVIVAL_FLAG", predictor, chemistry_survival)
...
```{r Length of ICU STAY length  by Laboratory Data (Blood) Blood Gas correlation matrix}
# Assuming `labs_blood_gas_LOS` contains numeric data and non-numeric columns to be excluded
correlation_data <- labs_blood_gas_LOS %>%
  select(where(is.numeric)) %>%
  select(-VISIT_NUM)
...
```{r Death Status by Laboratory Data (Blood Gas) stat tests}
# Define predictors to evaluate
predictors <- c("HEMOGLOBIN", "BASE_EXCESS", "FREE_CALCIUM", "LACTATE", "PCO2", "PH", "PO2", "POTASSIUM_WHOLE_BLOOD", "SODIUM_WHOLE_BLOOD", "CHLORIDE_WHOLE_BLOOD", "GLUCOSE", "PEEP", "TEMPERATURE", "OXYGEN", "OXYGEN_SATURATION", "TIDAL_VOLUME", "VENTILATION_RATE", "CALCULATED_BICARBONATE_WHOLE_BLOOD", "O2_FLOW", "METHEMOGLOBIN", "CARBOXYHEMOGLOBIN")
...
for (predictor in predictors) {
  ...
  result <- analyze_predictor_LOS(labs_blood_gas_LOS, predictor, LOS = "LOS", patient_id = "SUBJECT_ID_COMPOSE")
...
rm(table, results, predictors, predictor, labs_blood_gas_LOS, blood_gas, labs_blood_gas_combined)
```

#### 7.4.3.2. Hematology

```{r Length of ICU STAY length  by Laboratory Data (Blood) Hematology correlation matrix}
# Assuming `hematology_LOS` contains numeric data and non-numeric columns to be excluded
correlation_data <- hematology_LOS %>%
  select(where(is.numeric)) %>%
  select(-VISIT_NUM)
...
```{r Death Status by Laboratory Data Hematology stat tests}
# Define predictors to evaluate
predictors <- c("HEMOGLOBIN", "INR_PT", "LYMPHOCYTES", "MCH", "MCHC", "MCV", "MONOCYTES", "NEUTROPHILS", "EOSINOPHILS", "PLATELET_COUNT", "PTT", "RDW", "WHITE_BLOOD_CELLS", "FIBRINOGEN_FUNCTIONAL", "BASOPHILS", "SEDIMENTATION_RATE", "RETICULOCYTE_COUNT_AUTOMATED", "D_DIMER")
...
for (predictor in predictors) {
  # Perform assessment for the current predictor
  result <- analyze_predictor_LOS(hematology_LOS, predictor, LOS = "LOS", patient_id = "SUBJECT_ID_COMPOSE")
...
```{r Length of ICU STAY length  by Laboratory Data (Blood) Chemistry correlation matrix}
# Assuming `hematology_LOS` contains numeric data and non-numeric columns to be excluded
correlation_data <- chemistry_LOS %>%
  select(where(is.numeric)) %>%
  select(-VISIT_NUM)
...
```{r Death Status by Laboratory Data Chemistry stat tests}
# Define predictors to evaluate
predictors <- c("POTASSIUM", 
                      "SODIUM", 
                      "CREATININE", 
...
for (predictor in predictors) {
  # Perform assessment for the current predictor
  result <- analyze_predictor_LOS(chemistry_LOS, predictor, LOS = "LOS", patient_id = "SUBJECT_ID_COMPOSE")
...
data <- data.frame(
  Predictor = c(
    ...
    "Blood Gas", "Base excess", "Free calcium", "Lactate", "PCO2", "PH", "PO2", "Potassium whole blood", "Sodium whole blood", "Chloride whole blood", "PEEP", "Temperature", "Oxygen", "Oxygen saturation", "Tidal volume", "Ventilation rate", "Caclulated bicarbonate whole blood", "O2 flow", "Methehemoglobin", "Carboxyhemoglobin",
    "Hematology", "Hemoglobin", "INR PT", "Lymphocytes", "MCH", "MCHC", "MCV", "Monocytes", "Neutrophils", "Eosinophils", "Platelet count", "PTT", "RDW", "White blood cells", "Fibrinogen functional", "Basophils", "Sedimentation rate", "Reticulocyte count", "D-dimer",
    "Blood Chemistry", "Potassium", "Sodium", "Creatinine", "Chloride", "Urea nitrogen", "Bicarbonate", "Glucose", "Anion gap", "Magnesium", "Phosphate", "Calcium total", "Bilirubin total", "ALT", "AST", "CRP", "Trroponin T", "LD" ),
  ...
  Death_Status = c("", "+", "+", "+", "+", "+", "+", "+", "", "+", "+", "+", 
                   ...
                   "", "+", "+", "+", "+", "+", "+",
                   "", "+", "+", "+", "+", "+", "+", "-", "-", "-", "+", "+", "+", "+", "+", "+", "+", "+", "-", "-",
                   "", "HEMOGLOBIN", "INR_PT", "LYMPHOCYTES", "MCH", "MCHC", "MCV", "MONOCYTES", "NEUTROPHILS", "EOSINOPHILS", "PLATELET_COUNT", "PTT", "RDW", "WHITE_BLOOD_CELLS", "FIBRINOGEN_FUNCTIONAL", "BASOPHILS", "SEDIMENTATION_RATE", "RETICULOCYTE_COUNT_AUTOMATED", "D_DIMER",
                   "", "+", "+", "+", "+", "+", "+", "+", "+", "+", "+", "+", "+", "+", "+", "+", "+", "+"),
  ...
  LOS = c("", "-", "+", "+", "-", "+", "+", "-", "", "", "Not estimated", "Not estimated",
          ...
          "", "+", "-", "-", "-", "+", "+", "-", "-", "+", "-", "+", "+", "-", "+", "-", "+", "+", "-", "-",
          "", "HEMOGLOBIN", "INR_PT", "LYMPHOCYTES", "MCH", "MCHC", "MCV", "MONOCYTES", "NEUTROPHILS", "EOSINOPHILS", "PLATELET_COUNT", "PTT", "RDW", "WHITE_BLOOD_CELLS", "FIBRINOGEN_FUNCTIONAL", "BASOPHILS", "SEDIMENTATION_RATE", "RETICULOCYTE_COUNT_AUTOMATED", "D_DIMER",
          "", "Potassium", "Sodium", "Creatinine", "Chloride", "Urea nitrogen", "Bicarbonate", "Glucose", "Anion gap", "Magnesium", "Phosphate", "Calcium total", "Bilirubin total", "ALT", "AST", "CRP", "TRroponin T", "LD"))
