...
skimr::skim(patients$EXPIRE_FLAG)
skimr::skim(admissions$TIMETODEATH)
skimr::skim(icu_stays$LOS)
...
ggplot(admissions, aes(y = TIMETODEATH_DAYS)) +
...
ggplot(admissions, aes(x = TIMETODEATH_DAYS)) +
...
## 2.3.   Visualization Analysis by Outcome
...
combined_data <- admissions %>%
  left_join(patients, by = "SUBJECT_ID")

# Create a new age group column based on quintiles
age_breaks <- quantile(combined_data$AGE_AT_ADMISSION, probs = seq(0, 1, 0.2), na.rm = TRUE)
...
ggplot(combined_data, aes(x = GENDER, y = TIMETODEATH_DAYS, fill = GENDER)) +
  geom_boxplot() +
...
combined_data_los <- icu_stays %>%
  left_join(patients, by = "SUBJECT_ID") %>%
  left_join(admissions, by = c("SUBJECT_ID", "HADM_ID"))  # Ensure you join with admissions to get TIMETODEATH if needed
