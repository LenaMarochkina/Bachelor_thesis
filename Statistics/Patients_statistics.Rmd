...
procedures_with_duration_death_outcome_long <- procedures_with_duration_death_outcome %>%
  pivot_wider(
    ...
    values_fn = list(VALUE = ~ sum(.))  # Sum values if more than 1
  ) %>%
  select(-"NA") %>%
  rename(
    ARTERIAL_LINE = "ARTERIAL LINE",
    INVASIVE_VENTILATION = "INVASIVE VENTILATION",
    MULTI_LUMEN = "MULTI LUMEN",
    DIALYSIS_CRRT = "DIALYSIS - CRRT"
  ) %>%
  # All NULL to 0
  mutate(across(c("ARTERIAL_LINE", "INVASIVE_VENTILATION", "MULTI_LUMEN", "DIALYSIS_CRRT"), ~ as.numeric(as.character(.)))) %>%  
  mutate(across(c("ARTERIAL_LINE", "INVASIVE_VENTILATION", "MULTI_LUMEN", "DIALYSIS_CRRT"), ~ replace_na(., 0)))
...
death_status <- death_status %>%
  ...
  mutate(across(c("EXTUBATION", "INTUBATION", "CHEST_X_RAY"), ~ as.factor(.)))
...
survival_length <- admissions_survival_outcome %>%
  ...
  select(-AGE_QUINTILE, -AGE_GROUP, -SUBJECT_ID)  # Remove unnecessary columns
...
survival_length <- survival_length %>%
  left_join(prescriptions_survival_numbers, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL")) %>%
  select(-prescriptions_NUM)

 # add ATC_ label before column names
prescriptions_transformed_survival <- prescriptions_transformed_survival %>%
  rename_with(~ paste0("ATC_", .x), -c(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG, SURVIVAL))

survival_length <- survival_length %>%
  # add ATC_ before column names
  left_join(prescriptions_transformed_survival, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL")) %>%
  # All NA's change to 0 in columns from prescriptions_transformed_death_outcome
  mutate(across(starts_with("ATC"), ~ as.numeric(as.character(.)))) %>%  # Convert factors to numeric
  mutate(across(starts_with("ATC"), ~ replace_na(., 0))) %>%  # Replace NA with 0
  mutate(across(starts_with("ATC"), ~ as.factor(.)))
...
procedures_with_duration_survival_long <- procedures_with_duration_survival_outcome %>%
  select(SURVIVAL, SURVIVAL_FLAG, SUBJECT_ID_COMPOSE, LABEL, VALUE) %>%
  pivot_wider(
    names_from = LABEL,   # Column to turn into new column names
    values_from = VALUE,   # Column to populate the new columns
    values_fn = list(VALUE = ~ sum(.))  # Sum values if more than 1
...
survival_length <- survival_length %>%
  left_join(procedures_with_duration_survival_long, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL"))

survival_length <- survival_length %>%
  left_join(procedures_binary_survival_outcome_flags, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL")) %>%
  mutate(across(c("EXTUBATION", "INTUBATION", "CHEST_X_RAY"), ~ as.numeric(as.character(.)))) %>%  # Convert factors to numeric
  mutate(across(c("EXTUBATION", "INTUBATION", "CHEST_X_RAY"), ~ replace_na(., 0))) %>%  # Replace NA with 0
  mutate(across(c("EXTUBATION", "INTUBATION", "CHEST_X_RAY"), ~ as.factor(.)))
