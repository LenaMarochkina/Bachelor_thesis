...
``` {r Survival Analysis}
# General function to fit a Cox model and return hazard ratios, confidence intervals, and p-values
cox_model_summary <- function(survival_time, event_status, predictors, data) {
  ...
  # Create a data frame with hazard ratios, confidence intervals, and p-values
  hr_table <- data.frame(
    Variable = predictors,
    Hazard_Ratio = hr,
    Lower_CI = hr_ci[, 1],
    ...
    # Format p-values
  hr_table$p_value <- sapply(hr_table$p_value, function(p) {
    if (p < 0.0001) {
      return("< 0.0001")  # Handle very small p-values
    } else {
      return(formatC(p, format = "e", digits = 2))  # Format in scientific notation
    }
  })
...
hr_table <- cox_model_summary("SURVIVAL", "EXPIRE_FLAG", c("GENDER", "AGE_AT_ADMISSION"), patients_admissions)

# Display the hazard ratio table
print(hr_table)
```

**GENDER:** There is no statistically significant effect of gender on survival in your data. The p-value for gender is above the typical threshold of 0.05, and the confidence interval includes 1, indicating no strong evidence of an association.
