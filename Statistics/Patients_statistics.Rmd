...
```{r libraries, set colors, echo=FALSE}
library(tidyverse)
library(readr)
library(dplyr)
library(tidyr)
...
```{r Descriptive Stat functions, echo=FALSE}
...
``` {r Statictical hypothesis check, echo=FALSE}
...
```{r penalized COX Regression, echo=FALSE}
...
ggplot(admissions, aes(x = as.factor(SURVIVAL_FLAG), fill = as.factor(SURVIVAL_FLAG))) +
  ...
  scale_fill_manual(
    ...
    labels = c("0" = "Discharged alive", "1" = "In-hospital death")
    ) +
  labs(
   x = "Death Status", y = "Number of Patients", fill = "Death Status"
    ) +
  theme_custom
...
# Remove 0-day survival values first
...
plot <- ggplot(patients_survival_outcome, aes(x = AGE_AT_ADMISSION, y = SURVIVAL, color = GENDER)) +
...
summary_df <- patients_survival_outcome %>%
  group_by(AGE_QUINTILE, GENDER) %>%
  summarise(mean_survival = mean(SURVIVAL, na.rm = TRUE), .groups = "drop")

# Line plot
ggplot(summary_df, aes(x = AGE_QUINTILE, y = mean_survival, group = GENDER, color = GENDER)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  scale_color_manual(values = sample(color_palette, size = length(unique(summary_df$GENDER)))) +
  labs(
    x = "Age Quintile",
    y = "Mean Survival Time (Days)",
    color = "Gender"
  ) +
  theme_custom +
  theme(
...
patients_survival_outcome <- patients_survival_outcome %>%
  mutate(
    SURVIVAL_FLAG = as.numeric(as.character(SURVIVAL_FLAG))  # Convert to numeric
  )
...
rm(hr_table, fit_age_group, fit_gender)
```
...
rm(patients, merged_results, result_age, result_sex)
```
...
skimr::skim(admissions %>%
            ...
            mutate(
              SURVIVAL_FLAG = factor(SURVIVAL_FLAG, levels = c(0, 1))
            ))
```
...
for (var in categorical_vars) {
  p <- ggplot(admissions_death_outcome_long[admissions_death_outcome_long$Variable == var, ],
              aes(x = Value, fill = as.factor(SURVIVAL_FLAG))) +
    ...
    scale_fill_manual(values = sample(color_palette, size = length(unique(admissions$SURVIVAL_FLAG))),
                      labels = c("0" = "Survived", "1" = "Died")) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.08)))  # Adds 10% extra space above the highest bar

  plots[[var]] <- p
}

# Combine all plots into one grid with 2 columns
...
admissions_survival_outcome <- admissions %>%
  select(SUBJECT_ID, SUBJECT_ID_COMPOSE, SURVIVAL, SURVIVAL_FLAG, ADMISSION_TYPE, INSURANCE, RELIGION, MARITAL_STATUS, ETHNICITY) %>%
  mutate(
    SURVIVAL_FLAG = factor(SURVIVAL_FLAG, levels = c(0, 1))
    )
...
# Combine all plots into one grid (using 2 columns, adjust ncol as needed)
combined_plots <- grid.arrange(grobs = plots, ncol = 2)

# Save the combined plot to a file (adjust width/height as needed)
ggsave("combined_admission_plots.png", combined_plots, width = 10, height = 8)
...
admissions_los_outcome <- admissions %>%
  ...
  mutate(SURVIVAL_FLAG = factor(SURVIVAL_FLAG, levels = c(0, 1)))
...
rm(hr_table, fit_religion, fit_admission_type, fit_ethnicity, fit_insurance, fit_marital_status)
...
rm(admissions, merged_results, result_RELIGION, result_MARITAL_STATUS, result_ADMISSION_TYPE, result_INSURANCE, result_ETHNICITY)
```
...
diagnoses %>%
  ...
  mutate(`No data` = sum(is.na(Value)),
         `% by group` = (n / sum(n)) * 100) %>%
  ungroup() %>%
  filter(n >= 100) %>% # Keep only rows with at least 100 observations
  select(Variable, Value, n, `% by group`, `No data`) %>%
...
ggplot(high_priority, aes(x = reorder(LONG_TITLE, -n), y = n)) +
  ...
  labs(title = "High-Priority Diagnoses",
       x = "ICD9 Code",
       y = "Frequency") +
  theme_custom +
  coord_flip() +
...
# check SUBJECT_ID_COMPOSE duplicates
diagnoses_transformed_death_outcome %>%
  group_by(SUBJECT_ID_COMPOSE) %>%
  summarise(n = n()) %>%
  filter(n > 1)
...
icd9_summary <- diagnoses_transformed_death_outcome %>%
  ...
  scale_fill_manual(
    values = c("0" = color_palette[12], "1" = color_palette[1]),
...
ggplot(data_long, aes(x = ICD9_CODE, y = SURVIVAL, fill = as.factor(SURVIVAL_FLAG))) +
  ...
  # Mean points
  stat_summary(fun = median, geom = "point", shape = 20, size = 2, color = "black", position = position_dodge(width = 0.75)) +
  ...
  coord_flip() +
  scale_fill_manual(
    values = c("0" = color_palette[6], "1" = color_palette[10]),
    labels = c("Discharged alive", "In-hospital death")
  ) +
  theme_minimal() +
  labs(
    x = "ICD9 Codes",
  ...
  facet_wrap(
    ~ SURVIVAL_FLAG,
    labeller = as_labeller(c(`0` = "Discharged alive", `1` = "In-hospital death"))
  ) +
  theme(
...
# Merge survival outcome with diagnoses and recode SURVIVAL_FLAG
diagnoses_survival_outcome <- Survival_outcome %>%
  left_join(diagnoses, by = "SUBJECT_ID_COMPOSE") %>%
  select(SUBJECT_ID_COMPOSE, SURVIVAL, SURVIVAL_FLAG, DIAGNOSES_NUM, LONG_TITLE, SEQ_NUM) %>%
  mutate(SURVIVAL_FLAG = factor(SURVIVAL_FLAG, levels = c(0, 1)))

# Create a unique dataset and categorize DIAGNOSES_NUM into intervals
diagnoses_survival_numbers <- diagnoses_survival_outcome %>%
  ...
  mutate(
    DIAGNOSES_NUM = cut(DIAGNOSES_NUM,
                        breaks = c(0, 1, 3, 5, 10, 40),
                        labels = c("1", "1-3", "3-5", "6-10", ">10"),
                        right = FALSE)
  ) %>%
  droplevels()  # Remove any unused factor levels

# Fit the Kaplan-Meier model using the processed data
fit_diagnoses_num <- survfit(
  Surv(SURVIVAL, as.numeric(as.character(SURVIVAL_FLAG))) ~ DIAGNOSES_NUM,
  data = diagnoses_survival_numbers
)

# Plot the Kaplan-Meier curve using the same dataset as used for the model
ggsurvplot(
  fit_diagnoses_num,
  data = diagnoses_survival_numbers,   # Note: using diagnoses_survival_numbers here
  pval = TRUE,
  conf.int = TRUE,
  ggtheme = theme_custom,
  surv.median.line = "hv",
  legend.title = "Number of Diagnoses",
  title = "Survival Curve by Number of Diagnoses",
  palette = color_palette
)

diagnoses_survival_numbers <- diagnoses_survival_numbers %>%
  mutate(
...
rm(diagnoses_survival_numbers, fit_diagnoses_num, hr_table_number_diagnoses)
...
In contrast, several ICD-9 diagnoses showed strong associations with survival. For instance, ICD9 4861 (HR = 1.32), 58491 (HR = 1.49), 03891 (HR = 2.44), 4311 (HR = 2.13), and 518811 (HR = 2.30) were linked to higher risk of death. Conversely, ICD9 42401 (HR = 0.62), 414011 (HR = 0.51), and 42411 (HR = 0.59) were associated with better survival, confirming the prognostic relevance of specific conditions
...
rm(result_diagnoses_num, diagnosis_LOS_numbers)
```
...
isolations <- read.csv("../data/raw/cleaned/CALLOUT_clean.csv", stringsAsFactors = TRUE) %>%
  ...
  mutate(
    ...
    OUTCOMETIME = ymd_hms(as.character(OUTCOMETIME))  # convert OUTCOMETIME to POSIXct
  ) %>%
  select(-ROW_ID, -CALLOUT_OUTCOME, -HADM_ID) %>%
  distinct() %>%
  group_by(SUBJECT_ID_COMPOSE) %>%
  filter(OUTCOMETIME == max(OUTCOMETIME, na.rm = TRUE)) %>%
  ungroup()
...
ggplot(count_data, aes(x = Request_Type, y = Count, fill = factor(SURVIVAL_FLAG))) +
  ...
  labs(
       ...
       y = "Number of Patients",
       fill = "Survival Status") +
  scale_fill_manual(values = c(color_palette[8], color_palette[12]), labels = c("Discharged alive", "In-hospital death")) +
...
ggplot(isolations_long, aes(x = factor(Isolation_Flag), y = SURVIVAL, fill = factor(Isolation_Flag))) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  stat_summary(fun = median, geom = "point", shape = 21, size = 3, fill = "black", color = "black") +
...
rm(hr_table, fit_CDIFF, fit_MRSA, fit_VRE)
...
<!-- rm(isolations, merged_results, result_CDIFF, result_MRSA, result_VRE) -->
<!-- ``` -->
...
prescriptions_death_outcome %>%
  ...
  summarise(num_drugs = n_distinct(DRUG_NAME_GENERIC), .groups = "drop") %>%
  mutate(
    cap = quantile(num_drugs, 0.99, na.rm = TRUE),
    num_drugs_capped = pmin(num_drugs, cap)
  )
# Assuming Death_status_outcome contains death information and prescriptions has drug data
prescriptions_death_outcome_capped <- prescriptions_death_outcome %>%
  group_by(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG) %>%
  summarise(num_drugs = n_distinct(DRUG_NAME_GENERIC), .groups = "drop") %>%
  mutate(
    cap = quantile(num_drugs, 0.99, na.rm = TRUE),
  ...
  ggplot(prescriptions_death_outcome_capped, aes(x = num_drugs_capped, fill = factor(SURVIVAL_FLAG), color = factor(SURVIVAL_FLAG))) +
  geom_density(alpha = 0.4) +
  labs(

    x = "Number of Unique Drugs (Capped at 99th Percentile)",
    y = "Density",
    fill = "Survival Status",
...
prescriptions_survival_outcome <- Survival_outcome %>%
  ...
  distinct() %>%
  group_by(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG, SURVIVAL) %>%
  summarise(num_drugs = n_distinct(DRUG_NAME_GENERIC), .groups = "drop") %>%
  mutate(
    cap = quantile(num_drugs, 0.99, na.rm = TRUE),
    num_drugs_capped = pmin(num_drugs, cap)
...
ggplot(prescriptions_survival_outcome, aes(x = factor(SURVIVAL_FLAG), y = num_drugs_capped, fill = factor(SURVIVAL_FLAG))) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +
  geom_jitter(aes(color = factor(SURVIVAL_FLAG)), width = 0.2, alpha = 0.3, size = 1.2) +
  scale_fill_manual(
    values = c("0" = color_palette[10], "1" = color_palette[12]),
    labels = c("0" = "Discharged Alive", "1" = "In-hospital Death")
  ) +
  scale_color_manual(
    values = c("0" = color_palette[10], "1" = color_palette[12]),
  ...
  labs(
    title = "Number of Unique Drugs per Patient by Survival Status",
    x = "Survival Status",
...
ggplot(prescriptions_survival_outcome , aes(x = num_drugs_capped, y = SURVIVAL)) +
  geom_jitter(alpha = 0.3, color = color_palette[4], size = 1.2) +
  geom_smooth(method = "lm", color = "yellow", se = FALSE, linewidth = 1.2) +  # Add red regression line
  labs(
    x = "Number of Unique Drugs (Capped at 99)",
    y = "Survival Time (Days)"
  ) +
  theme_custom +
  theme(
...
# Bin survival time (e.g. in 10-day intervals)
prescriptions_mean_drugs <- prescriptions_survival_outcome  %>%
  mutate(survival_bin = cut(SURVIVAL, breaks = seq(0, max(SURVIVAL, na.rm = TRUE), by = 3), right = FALSE)) %>%
  group_by(survival_bin) %>%
  summarise(
    mean_drugs = mean(num_drugs_capped, na.rm = TRUE),
    count = n(),
    .groups = "drop"
  )

# Plot
ggplot(prescriptions_mean_drugs, aes(x = survival_bin, y = mean_drugs, group = 1)) +
  geom_line(color = "#2c7fb8", linewidth = 1.2) +
  geom_point(color = "#2c7fb8", size = 2) +
  labs(
    title = "Mean Number of Drugs by Survival Time",
    x = "Survival Time (Binned in 10-Day Intervals)",
    y = "Mean Number of Unique Drugs"
  ) +
  theme_custom +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1)
...
rm(fit_prescriptions_num, hr_table_number_drugs)
...
labs_blood <- read.csv("../data/raw/cleaned/LABEVENTS_clean_blood.csv", stringsAsFactors = TRUE) %>%
  filter(FLUID == "BLOOD") %>%
  mutate(across(where(is.character), as.factor))  %>%
  filter(SUBJECT_ID %in% subject_ids_adults) %>%
    filter(HADM_ID != 0 ) %>%
...
latex_table <- labs_blood_gas_combined_last %>%
  select(where(is.numeric)) %>%
  summarise(across(everything(), statistics)) %>%
  pivot_longer(cols = everything(), names_sep = "__", names_to = c("Stat", "Variable")) %>%
  pivot_wider(names_from = Stat, values_from = value) %>%
  rename(`Variable` = Variable) %>%
  kable(format = "latex", booktabs = TRUE, digits = 2, caption = "Summary Statistics for Chemistry Lab Variables", label = "tab:chemistry_summary")

# Step 2: Save to .tex file
writeLines(latex_table, "chemistry_summary_table.tex")
...
# Summarize, reshape, and prepare for LaTeX export
hematology_summary <- hematology_combined_last %>%
  select(where(is.numeric)) %>%
  summarise(across(everything(), statistics)) %>%
  pivot_longer(cols = everything(), names_sep = "__", names_to = c("Variable", "Stat")) %>%
  pivot_wider(names_from = Stat, values_from = value)
...
latex_code <- kable(
  ...
  booktabs = TRUE,
  digits = 2,
  caption = "Summary Statistics for Hematology Lab Variables",
  label = "tab:hematology_summary"
)

# Save to .tex file
writeLines(latex_code, "hematology_summary_table.tex")
...
chemistry_summary <- chemistry_combined_last %>%
  select(where(is.numeric)) %>%
  summarise(across(everything(), statistics)) %>%
  pivot_longer(cols = everything(), names_sep = "__", names_to = c("Variable", "Stat")) %>%
  pivot_wider(names_from = Stat, values_from = value)
...
latex_code <- kable(
  ...
  format = "latex",
  booktabs = TRUE,
  caption = "Summary Statistics for Chemistry Lab Variables",
  label = "tab:chemistry_summary"
)

# Save LaTeX code to file
writeLines(latex_code, "chemistry_summary_table.tex")
...
rm(melted_data, filtered_data, numeric_vars)
```
...
# Split the list of variables into 2 parts
blood_test_params_1 <- blood_test_params[1:9]
blood_test_params_2 <- blood_test_params[10:18]
blood_test_params_3 <- blood_test_params[19:25]

# Helper function to generate hexbin plots
generate_hex_plots <- function(param_list, data) {
  lapply(param_list, function(param) {
    df <- data %>% filter(!is.na(.data[[param]]))

    ggplot(df, aes(x = SURVIVAL, y = .data[[param]])) +
      stat_binhex(aes(fill = ..count..), bins = 50) +
...
# Create plot objects
plots_1 <- generate_hex_plots(blood_test_params_1, labs_blood_gas_survival_last)
plots_2 <- generate_hex_plots(blood_test_params_2, labs_blood_gas_survival_last)
plots_3 <- generate_hex_plots(blood_test_params_3, labs_blood_gas_survival_last)

# Combine plots into 2 grids
grid_1 <- wrap_plots(plots_1, ncol = 3)
grid_2 <- wrap_plots(plots_2, ncol = 3)
grid_3 <- wrap_plots(plots_3, ncol = 3)
...
# Save png
ggsave("bg_survival_1.png", plot = grid_1, width = 12, height = 8)
ggsave("bg_survival_2.png", plot = grid_2, width = 12, height = 8)
ggsave("bg_survival_3.png", plot = grid_3, width = 12, height = 8)
...
# Split the list of variables into 2 parts
blood_test_params_1 <- blood_test_params[1:12]
blood_test_params_2 <- blood_test_params[13:21]
...
# Create plot objects
plots_1 <- generate_hex_plots(blood_test_params_1, hematology_survival_last)
plots_2 <- generate_hex_plots(blood_test_params_2, hematology_survival_last)
...
# Save png
ggsave("h_survival_1.png", plot = grid_1, width = 12, height = 8)
ggsave("h_survival_2.png", plot = grid_2, width = 12, height = 8)
...
# Split the list of variables into 2 parts
blood_test_params_1 <- blood_test_params[1:9]
blood_test_params_2 <- blood_test_params[10:17]
...
# Create plot objects
plots_1 <- generate_hex_plots(blood_test_params_1, chemistry_survival_last)
plots_2 <- generate_hex_plots(blood_test_params_2, chemistry_survival_last)
...
# Save png
ggsave("ch_survival_1.png", plot = grid_1, width = 12, height = 8)
ggsave("ch_survival_2.png", plot = grid_2, width = 12, height = 8)
...
# Define predictors to evaluate
predictors <- c("HEMOGLOBIN", "ALVEOLAR_ARTERIAL_GRADIENT", "CALCULATED_TOTAL_CO2", "FREE_CALCIUM", "LACTATE", "PCO2", "PH", "PO2", "POTASSIUM_WHOLE_BLOOD", "SODIUM_WHOLE_BLOOD", "CHLORIDE_WHOLE_BLOOD", "PEEP", "TEMPERATURE", "OXYGEN", "OXYGEN_SATURATION", "TIDAL_VOLUME", "VENTILATION_RATE", "CALCULATED_BICARBONATE_WHOLE_BLOOD", "O2_FLOW", "METHEMOGLOBIN", "CARBOXYHEMOGLOBIN")
...
# Define predictors to evaluate
predictors <- c("HEMOGLOBIN", "RED_BLOOD_CELLS", "INR_PT", "MCV", "MCHC", "MONOCYTES", "NEUTROPHILS", "EOSINOPHILS", "PLATELET_COUNT", "PTT", "PT", "RDW", "WHITE_BLOOD_CELLS", "FIBRINOGEN_FUNCTIONAL", "BASOPHILS", "SEDIMENTATION_RATE", "RETICULOCYTE_COUNT_AUTOMATED", "D_DIMER")
...
rm(predictors, hr_results, hr_table, hr_result)
```
...
procedures_binary <- procedures %>%
  ...
  filter(n_distinct(VALUE) == 1) %>%  # remove if both 0 and 1 exist
  ungroup()
...
<!-- rm(procedures, procedures_binary, procedures_long, results_procedures_without_duration_LOS, results_procedures_with_duration_LOS) -->
<!-- ``` -->
...
## 9.1. Read data
``` {r read vital signs}
vital <- read.csv("../data/raw/cleaned/VITAL_SIGNS_clean.csv", stringsAsFactors = TRUE) %>%
  mutate(across(where(is.character), as.factor))  %>%
  filter(SUBJECT_ID %in% subject_ids_adults) %>%
  mutate(SUBJECT_ID = as.factor(SUBJECT_ID))

skimr::skim(vital %>%
              select(-SUBJECT_ID, -SUBJECT_ID_COMPOSE))
```
## 9.2. Descriptive Statistics

```{r transform data vitals}

# Pivot wider with new short names as column headers
vital_pivoted <- vital %>%
  pivot_wider(
    names_from = LABEL,
...
```
## 9.3. Visualisation Analysis by Outcome
### 9.3.1. Outcome 1: Death Status by Vital Signs Data

```{r Death Status by Vital Signs Data}

# Reshape first — assuming 'vital_pivoted' uses short column names like ABP_SYS
vitals_long <- vital_pivoted %>%
  pivot_longer(
    cols = -c(SUBJECT_ID, SUBJECT_ID_COMPOSE),  # Exclude ID columns
    names_to = "Variable",                      # New column for variable names
    values_to = "Value"                        # New column for values
  )

vitals_death_status <- Death_status_outcome %>%
  left_join(vitals_long, by = c("SUBJECT_ID","SUBJECT_ID_COMPOSE")) %>%
  mutate(SURVIVAL_FLAG = as.factor(SURVIVAL_FLAG))  # ensure it's factor for fill aesthetics

# Now you can plot
ggplot(vitals_death_status, aes(x = SURVIVAL_FLAG, y = Value, fill = SURVIVAL_FLAG)) +
  geom_violin(trim = FALSE, alpha = 0.6) +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) +
  facet_wrap(~ Variable, scales = "free_y", ncol = 3) +
  scale_fill_manual(values = c("0" = color_palette[7], "1" = color_palette[12]), labels = c("Survived", "Died")) +
  labs(
    title = "Distribution of Vital Signs by Death Status",
    x = "Death Status",
    y = "Value"
  ) +
...
```
### 9.3.2. Outcome 2: Length of Survival by Vital Signs Data

```{r Length of Survival by Vital Signs Data}
library(patchwork)
vitals_survival_outcome <- Survival_outcome %>%
  left_join(vitals_long, by = "SUBJECT_ID_COMPOSE")
  
plots <- vitals_long %>%
  left_join(Survival_outcome, by = "SUBJECT_ID_COMPOSE") %>%
  group_split(Variable) %>%
  map(~{
    ggplot(.x, aes(x = Value, y = SURVIVAL)) +
      geom_point(alpha = 0.1, color = color_palette[10]) +
      geom_smooth(method = "loess", color = "#1B9E77", se = FALSE) +
      labs(
...
# Combine into grid of plots (adjust ncol if needed)
...
### 9.4.1. Outcome 1: Death Status by Vital Signs Data

```{r Death Status by Vital Signs Data Exploratory Analysis}
# Perform statistical tests for each vital sign
vitals_death_status <- vitals_death_status %>%
  mutate(SURVIVAL_FLAG = as.factor(SURVIVAL_FLAG))  # Ensure it's a factor for statistical tests

# Pivot Variable
vitals_death_status_long <- vitals_death_status %>%
  pivot_wider(
    names_from = Variable,
    values_from = Value
  ) %>% 
...
result_ABP_DIA <- perform_statistical_tests(vitals_death_status_long, "SURVIVAL_FLAG", "ABP_DIA", "Arterial BP Diastolic")
result_ABP_SYS <- perform_statistical_tests(vitals_death_status_long, "SURVIVAL_FLAG", "ABP_SYS", "Arterial BP Systolic")
result_ABP_MEAN <- perform_statistical_tests(vitals_death_status_long, "SURVIVAL_FLAG", "ABP_MEAN", "Arterial BP Mean")
result_NBP_DIA <- perform_statistical_tests(vitals_death_status_long, "SURVIVAL_FLAG", "NBP_DIA", "Non-Invasive BP Diastolic")
result_NBP_SYS <- perform_statistical_tests(vitals_death_status_long, "SURVIVAL_FLAG", "NBP_SYS", "Non-Invasive BP Systolic")
result_NBP_MEAN <- perform_statistical_tests(vitals_death_status_long, "SURVIVAL_FLAG", "NBP_MEAN", "Non-Invasive BP Mean")
result_Heart_Rate <- perform_statistical_tests(vitals_death_status_long, "SURVIVAL_FLAG", "Heart_Rate", "Heart Rate")
result_Resp_Rate <- perform_statistical_tests(vitals_death_status_long, "SURVIVAL_FLAG", "RESP_RATE", "Respiratory Rate")
result_Temperature <- perform_statistical_tests(vitals_death_status_long, "SURVIVAL_FLAG", "TEMP", "Temperature")



# Merge the results
merged_results <- bind_rows(result_ABP_DIA, result_ABP_SYS, result_ABP_MEAN,
                             result_NBP_DIA, result_NBP_SYS, result_NBP_MEAN,
                             result_Heart_Rate, result_Resp_Rate,
                             result_Temperature) 
...
### 9.4.2. Outcome 2: Length of Survival by Procedures

```{r Length of Survival by Procedures Exploratory analysis}
vitals_survival_outcome <- vitals_survival_outcome %>%
  mutate(
    SURVIVAL_FLAG = as.numeric(as.character(SURVIVAL_FLAG))  # Convert to numeric
  ) %>%
  pivot_wider(
...
# Generate Kaplan-Meier survival curve by all variables
fit_ABP_DIA <- survfit(Surv(SURVIVAL, SURVIVAL_FLAG) ~ ABP_DIA, data = vitals_survival_outcome)
fit_ABP_SYS <- survfit(Surv(SURVIVAL, SURVIVAL_FLAG) ~ ABP_SYS, data = vitals_survival_outcome)
fit_ABP_MEAN <- survfit(Surv(SURVIVAL, SURVIVAL_FLAG) ~ ABP_MEAN, data = vitals_survival_outcome)
fit_NBP_DIA <- survfit(Surv(SURVIVAL, SURVIVAL_FLAG) ~ NBP_DIA, data = vitals_survival_outcome)
fit_NBP_SYS <- survfit(Surv(SURVIVAL, SURVIVAL_FLAG) ~ NBP_SYS, data = vitals_survival_outcome)
fit_NBP_MEAN <- survfit(Surv(SURVIVAL, SURVIVAL_FLAG) ~ NBP_MEAN, data = vitals_survival_outcome)
fit_Heart_Rate <- survfit(Surv(SURVIVAL, SURVIVAL_FLAG) ~ Heart_Rate, data = vitals_survival_outcome)
fit_Resp_Rate <- survfit(Surv(SURVIVAL, SURVIVAL_FLAG) ~ RESP_RATE, data = vitals_survival_outcome)
fit_Temperature <- survfit(Surv(SURVIVAL, SURVIVAL_FLAG) ~ TEMP, data = vitals_survival_outcome)

hr_table <- HR_assessment("SURVIVAL", "SURVIVAL_FLAG", c("ABP_DIA", "ABP_SYS",
                                                         "ABP_MEAN", "NBP_SYS", "NBP_DIA", "NBP_MEAN", "Heart_Rate", "RESP_RATE", "TEMP"), vitals_survival_outcome)
...
## 10.1. Dataset for Future Modeling Death Outcome
...
prescriptions_number <- prescriptions_number %>%
  mutate(
    SURVIVAL_FLAG = factor(SURVIVAL_FLAG, levels = c(0, 1))
  )

isolations_death_outcome <- isolations_death_outcome %>%
  mutate(
    SURVIVAL_FLAG = factor(SURVIVAL_FLAG, levels = c(0, 1))
  )

procedures_with_duration_death_outcome <- procedures_with_duration_death_outcome %>%
  mutate(
    SURVIVAL_FLAG = factor(SURVIVAL_FLAG, levels = c(0, 1))
  )
...
death_status <- admissions_death_outcome %>%
  ...
  select(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG, AGE_AT_ADMISSION, GENDER)
...
labs_blood_gas_death_outcome_last <- labs_blood_gas_death_outcome_last %>%
  rename_with(~ paste0("BG_LAST_", .x), -c(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG))
...
# Add to each column except SUBJECT_ID_COMPOSE and SURVIVAL_FLAG prefix LAST_
hematology_death_outcome_last <- hematology_death_outcome_last %>%
  rename_with(~ paste0("H_LAST_", .x), -c(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG))
...
# Add to each column except SUBJECT_ID_COMPOSE and SURVIVAL_FLAG prefix LAST_
chemistry_death_outcome_last <- chemistry_death_outcome_last %>%
  rename_with(~ paste0("CH_LAST_", .x), -c(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG)) 
...
# multicolinearity 
death_status <- death_status %>%
  select(-BG_LAST_BASE_EXCESS, -BG_LAST_HEMATOCRIT_CALCULATED, -BG_LAST_REQUIRED_O2, 
         -H_LAST_HEMATOCRIT, -H_LAST_RED_BLOOD_CELLS, -H_LAST_PT, -H_LAST_LYMPHOCYTES, -H_LAST_MCH)
...
## 10.2. Dataset for Future Modeling Survival Time

```{r Prepare DataSet for Future Modeling Survival Time}
diagnoses_survival_outcome <- diagnoses_survival_outcome %>%
  mutate(
    SURVIVAL_FLAG = as.numeric(as.character(SURVIVAL_FLAG))
  )

survival_length <- admissions_survival_outcome %>%
  left_join(patients_survival_outcome, by = c("SUBJECT_ID", "SURVIVAL_FLAG", "SURVIVAL")) %>%
  select(-AGE_QUINTILE, -AGE_GROUP, -SUBJECT_ID, -MARITAL_STATUS, -INSURANCE, -ADMISSION_TYPE, -RELIGION, -ETHNICITY)  # Remove unnecessary columns
...
survival_length <- survival_length %>%
  left_join(diagnoses_survival_outcome, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL"))

survival_length <- survival_length %>%
  left_join(diagnoses_transformed_survival, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL")) %>%
  # Replace all NAs with 0
  mutate(across(starts_with("ICD9"), ~ as.numeric(as.character(.)))) %>%  # Convert factors to numeric
  mutate(across(starts_with("ICD9"), ~ replace_na(., 0))) %>%  # Replace NA with 0
  mutate(across(starts_with("ICD9"), ~ as.factor(.)))
...
survival_length <- survival_length %>%
  left_join(isolations_survival_outcome, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL")) %>%
  # All NA's change to 0 
  mutate(across(starts_with("REQUEST"), ~ as.numeric(as.character(.)))) %>%  # Convert factors to numeric
  mutate(across(starts_with("REQUEST"), ~ replace_na(., 0))) %>%  # Replace NA with 0
  mutate(across(starts_with("REQUEST"), ~ as.factor(.)))
...
# Add to each column except SUBJECT_ID_COMPOSE and SURVIVAL_FLAG prefix LAST_
labs_blood_gas_survival_last <- labs_blood_gas_survival_last %>%
  rename_with(~ paste0("BG_LAST_", .x), -c(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG, SURVIVAL))

survival_length <- survival_length %>%
  left_join(labs_blood_gas_survival_last, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL"))

# Add to each column except SUBJECT_ID_COMPOSE and SURVIVAL_FLAG prefix LAST_
hematology_survival_last <- hematology_survival_last %>%
  rename_with(~ paste0("H_LAST_", .x), -c(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG, SURVIVAL))

survival_length <- survival_length %>%
  left_join(hematology_survival_last, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL"))

# Add to each column except SUBJECT_ID_COMPOSE and SURVIVAL_FLAG prefix LAST_
chemistry_survival_last <- chemistry_survival_last %>%
  rename_with(~ paste0("CH_LAST_", .x), -c(SUBJECT_ID_COMPOSE, SURVIVAL_FLAG, SURVIVAL)) 

survival_length <- survival_length %>%
  left_join(chemistry_survival_last, by = c("SUBJECT_ID_COMPOSE", "SURVIVAL_FLAG", "SURVIVAL")) %>%
  # All NA's change to 0 in columns from prescriptions_transformed_death_outcome
  mutate(across(starts_with("LAST"), ~ as.numeric(as.character(.)))) %>%  # Convert factors to numeric
  mutate(across(starts_with("LAST"), ~ replace_na(., 0))) %>%  # Replace NA with 0
  mutate(across(starts_with("LAST"), ~ as.factor(.)))
...
# Save to CSV to directory prepared_to_prediction
write_csv(survival_length, "../data/prepared_to_prediction/Survival.csv")
